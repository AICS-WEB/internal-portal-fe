...
export default function FilesPage({ data, currentUser, actions }) {
  ...
  const rows = useMemo(() => {
    return data.sharedFiles
      .filter((file) => actions.canAccess(file))
      .filter((file) => category === "all" || file.category === category);
  }, [actions, category, data.sharedFiles]);
      ...
      render: (file) => {
        ...
        return (
          <div className="table-actions">
            ...
            {canManage ? <>
              <Button size="sm" variant="secondary" onClick={() => actions.openEdit("sharedFiles", file)}>버전 업데이트</Button>
              <Button size="sm" variant="danger" onClick={() => actions.deleteItem("sharedFiles", file.id, "공용 파일")}>삭제</Button>
            </> : null}
          </div>
        );
      },
