...
export function readSession() {
  try {
    ...
    if (session?.lastActivity && Date.now() - session.lastActivity > SESSION_TIMEOUT_MS) {
      clearSession();
...
export function touchSessionActivity() {
  ...
  if (session) saveSession({ ...session, lastActivity: Date.now() });
}
...
export async function apiRequest(path, options = {}) {
  ...
  const session = readSession();
  if (auth && session) touchSessionActivity();
