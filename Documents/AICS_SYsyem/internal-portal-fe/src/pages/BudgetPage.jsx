    ...
    { key: "status", header: "상태", render: (expense) => <Badge value={expense.status} /> },
    {
      ...
      render: (expense) => (
        <div className="table-actions">
          ...
          ) : canManage && ["approved", "rejected"].includes(expense.status) ? (
            <Button size="sm" variant="secondary" onClick={() => actions.updateItem("expenses", expense.id, { status: "pending" })}>재검토</Button>
          ) : "-"}
        </div>
      ),
    },
